...
from frisk.core.models import Evidence, Finding, Inventory, Item, Severity, iter_string_leaves
    ...
    def run(self, inventory: Inventory) -> list[Finding]:
        ...
        for item in inventory.items:
            for field_path, text in iter_string_leaves(item):
                findings.extend(self._scan(item, field_path, text))
        return findings

    def _scan(self, item: Item, field_path: str, text: str) -> list[Finding]:
        out: list[Finding] = []
