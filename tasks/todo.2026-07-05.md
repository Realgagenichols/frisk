...
## 6. Polish & gate
- [x] 6.1 README: honeypot section (what's seeded, what's detected, D8 severities) — R24
- [x] 6.2 `uv run ruff check .` clean + full `uv run pytest` green — N3
- [x] 6.3 `/review --fix` final gate; check M3 milestone box in SPEC.md; update STATUS.md
- [x] 6.4 Commit per section, staging by explicit path (Pattern 15 — no `git add -A`)

## Review

**M3 complete.** R24 delivered as a `D8` behavioral honeypot in `frisk/sandbox/honeypot.py`
(kept out of `frisk/core` to preserve Pyodide purity — R23/N1). 210 tests pass, ruff clean.

Per-section reviews (each fixed before proceeding):
- §1 seeding — probe now validates its own atime pin (setup failure was masquerading as
  capability success).
- §2 inspection — ENOTDIR reclassified as tamper (was a non-gating INFO error, downgrading
  real tampering).
- §3 canary scan — serverInfo offset switched to bytes (Evidence contract), field=raw pinned.
- §4 CLI wiring — verify's honeypot stderr line C0-escaped (item_ref can be a
  server-controlled tool name — R15).
- §5 fixtures/e2e — clean; reviewer confirmed no S3 leak via detector snippets (D2 is the
  only snippet source and doesn't match the thief's credential payload).

Final gate: core purity verified (no `frisk.sandbox` import in `frisk/core`), `_honeypot_line`
is the only Finding sink outside the renderers and it escapes, SPEC R24 + M3 checkboxes
consistent with passing tests. 5 lessons captured.
